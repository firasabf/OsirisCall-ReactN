...
class Video extends Component {
  constructor(props) {
    ...
    const config = {                           
      ...
      channelProfile: 0,                       
      videoEncoderConfig: {                     //Video feed encoder settings
        width: 720,
        height: 1080,
        bitrate: 1,
        frameRate: FPS30,
  ...
  videoView() {
    return (
      <View style={{ flex: 1 }}>
        ...
        {
          !this.state.vidMute                                              //local video
            ? <AgoraView style={styles.localVideoStyle} zOrderMediaOverlay={true} showLocalVideo={true} mode={1} />
            : <View />
        }
        <View style={styles.buttonBar}>
        <TouchableOpacity style={styles.iconStyle} backgroundColor="#0093E9" onPress={() => this.toggleAudio()}>
        <Text>microphone</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.iconStyle} backgroundColor="#0093E9" onPress={() => this.endCall()}>
        <Text>endCall</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.iconStyle} backgroundColor="#0093E9" onPress={() => this.toggleVideo()}>
        <Text>toggleVideo</Text>
        </TouchableOpacity>
